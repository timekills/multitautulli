...
class Config(object):
    _CONFIG_DEFINITIONS = {
        ...
        'HOME_SECTIONS': (list, 'General', ['server_status', 'current_activity', 'watch_stats', 'library_stats', 'recently_added']),
        ...
        'MONITOR_RCLONE': (int, 'Monitoring', 0),
    ...
    def reload(self):
        """
        Invokes the reload() call on the _config object.
        """
...
class ServerConfig(Config):
    ...
    _CONFIG_DEFINITIONS = {
        ...
        'PMS_TOKEN': (str, 'PMS', ''),
        'PMS_IDENTIFIER': (str, 'PMS', '127.0.0.1'),
        'PMS_IS_ENABLED': (int, 'PMS', 0),
        'PMS_IS_DELETED': (int, 'PMS', 0),
        'PMS_IS_REMOTE': (int, 'PMS', 0),
        'PMS_IS_CLOUD': (int, 'PMS', 0),
        'PMS_SSL': (int, 'PMS', 0),
        'PMS_SSL_PREF': (int, 'PMS', 1),
        'PMS_URI': (str, 'PMS', ''),
        'PMS_URL': (str, 'PMS', ''),
        ...
        'MONITOR_RCLONE_MOUNT': (int, 'Monitoring', 0),
        'RCLONE_USER': (str, 'RCLONE', ''),
        'RCLONE_PASS': (str, 'RCLONE', ''),
        'RCLONE_MOUNTDIR': (str, 'RCLONE', ''),
        'RCLONE_TMPDIR': (str, 'RCLONE', ''),
        'RCLONE_TESTFILE': (str, 'RCLONE', ''),
        'RCLONE_PORT': (int, 'RCLONE', 5572),
        'RCLONE_SSL': (int, 'RCLONE', 0),
        'RCLONE_SSL_HOSTNAME': (str, 'RCLONE', ''),
        'REFRESH_LIBRARIES_INTERVAL': (int, 'Monitoring', 12),
        'REFRESH_LIBRARIES_ON_STARTUP': (int, 'Monitoring', 1),
    }

    boolKeys = ['PMS_IS_ENABLED',
                'PMS_IS_REMOTE',
                'PMS_IS_CLOUD',
                'PMS_SSL',
                'PMS_IS_DELETED',
    ...
    def __init__(self, pms=None):
        ...
        self._config = {}
        for key in self._CONFIG_DEFINITIONS:
            super(ServerConfig, self).check_setting(key)

        if pms:
            self.process_kwargs(pms)

    def process_kwargs(self, kwargs):
        for key, value in kwargs.items():
            if key.upper() in self.boolKeys:
                kwargs[key] = bool_int(value)
        super(ServerConfig, self).process_kwargs(kwargs)
        self.write()

    def __getattr__(self, key):
        return super(ServerConfig, self).__getattr__(key)

    def __setattr__(self, key, value):
        if key.upper() in self.boolKeys:
            value = bool_int(value)
        super(ServerConfig, self).__setattr__(key, value)
        if key in self._CONFIG_DEFINITIONS.keys():
            monitor_db = database.MonitorDatabase()
            monitor_db.action('UPDATE servers SET %s = ? '
                              'WHERE id = ? ' % key.lower(),
                              args=[value, self.ID])
    ...
    def write(self):
        ...
        values_dict = {}
        for key in self._CONFIG_DEFINITIONS.keys():
            values_dict[key.lower()] = super(ServerConfig, self).__getattr__(key)
